    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white border p-2 shadow text-sm">
          ...
          {node.color === "green" && <div>Type: Revenue GL ✅</div>}
          {node.color === "red" && <div>Type: Blocked Revenue ❌</div>}
          {node.color === "#8884d8" && <div>Type: Inventory GL</div>}
          {node.color === "#82ca9d" && <div>Type: COGS GL</div>}
          {node.color === "#ccc" && <div>Type: Material or Profit Center</div>}
        </div>
      );
    }
    return null;
